...
自分の技量不足を自覚したので、私はHTMLの中に `<script>` タグでWebSocket APIをドライブするカスタムなJavaScriptを書くやり方と、もう一つHtmxのWebSocket拡張を利用するやり方との、二通りのWebSocketのデモを実装した。Vanilla JavaScriptによる実装を作って動かすことに成功して、ようやくHtmx WebSocket Extensionを理解することができた。ちょっと難儀でした。
